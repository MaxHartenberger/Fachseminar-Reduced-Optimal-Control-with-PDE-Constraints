...
Outputs under test_meshing/out_mesh/ by default:
- mesh.msh (Gmsh)
...
def build_mesh(cx=0.5, cy=0.5, r=0.1, h=0.03, refine_circle=True, outdir="test_meshing/out_mesh"):
    import gmsh
    import meshio
    ...
    # Identify surfaces (2D) for physical groups
    surfs = [tag for (dim, tag) in gmsh.model.getEntities(dim=2)]
    # Classify by area: the small area is the circle (~pi r^2)
...
if __name__ == "__main__":
    ...
    ap.add_argument("--outdir", default="test_meshing/out_mesh")
    args = ap.parse_args()

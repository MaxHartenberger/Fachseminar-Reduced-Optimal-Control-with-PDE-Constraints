...
\paragraph{Control region and indicator.}
Let the control region be the closed disk
\begin{equation}
  \omega \;=\; \big\{ x\in\Omega : \|x - c\|_2 \le r \big\},\quad c=\big(\tfrac{1}{2},\tfrac{1}{2}\big),\; r\in\big(0,\tfrac{1}{2}\big).
\end{equation}
Its characteristic (indicator) function $\chi_{\omega}:\Omega\to\{0,1\}$ is
\begin{equation}
  ...
  \end{cases}
\end{equation}
Consequently, for any integrable $\phi$ we have $\int_{\Omega} \chi_{\omega}\,\phi\,dx = \int_{\omega} \phi\,dx$, so the right-hand side of the weak form restricts the control action to $\omega$.
...
The FE discretization produces the following matrices (see the separate \href{run:report\_discretization.pdf}{Discretization Note} for exact code line references and details):
\begin{center}
\begin{tabular}{llp{0.55\textwidth}}
\hline
	extbf{Matrix} & \textbf{Symbol} & \textbf{Definition} \\
\hline
Stiffness & $A$ & $A_{ij} = \int_{\Omega} \nabla\varphi_i\!\cdot\!\nabla\varphi_j\,dx$ (Dirichlet BCs applied). \\
State Mass & $M$ & $M_{ij} = \int_{\Omega} \varphi_i\,\varphi_j\,dx$. \\
Control Mass & $M_U$ & $(M_U)_{ij} = \int_{\Omega} \varphi_i\,\varphi_j\,dx$ (inner product in $U_h$). \\
Control Coupling & $B$ & $B_{ij} = \int_{\omega} \varphi_i\,\varphi_j\,dx$ (restriction by $\chi_{\omega}$). \\
\hline
\end{tabular}
\end{center}
